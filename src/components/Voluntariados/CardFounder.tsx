...
export default function CardFounder({ name, job, img, url }: CardFounderProps) {
  return (
    <article className="relative h-110 py-10 rounded-2xl px-5 max-w-80 w-full group cursor-pointer transition-all duration-500 ease-out hover:scale-105 hover:shadow-2xl transform" style={{ background: 'var(--gradient-st)' }}>
      ...
      <div className="absolute inset-0 rounded-2xl pointer-events-none bg-gradient-to-r from-white/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
      ...
      <div className="absolute inset-0 rounded-2xl pointer-events-none border-2 border-transparent group-hover:border-white/30 transition-all duration-500"></div>
    </article>
  )
} 
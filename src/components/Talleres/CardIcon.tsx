...
export default function CardIcon({ children, number, description, isMayor500 }: CardIconProps) {
  ...
  return (
    <article ref={ref} className="flex flex-col justify-between items-center gap-1 min-w-50 w-full max-w-70 p-4 rounded-xl transition-all duration-300 ease-in-out hover:scale-102 hover:shadow-md hover:bg-white active:scale-102 active:shadow-md active:bg-white cursor-pointer group">
      <div className="bg-st flex items-center justify-center p-4 rounded-full text-white mb-3 h-18 w-18 transition-all duration-300 group-hover:scale-105 group-active:scale-105 ">
        {children}
      </div>
      <h3 className="text-4xl font-bold transition-all duration-300 group-hover:text-st group-active:text-st">{accelerator}<span>+</span></h3>
      <p className="text-lg text-gray800 transition-all duration-300 group-hover:text-gray-600 group-active:text-gray-600">{description}</p>
    </article>
  )
}
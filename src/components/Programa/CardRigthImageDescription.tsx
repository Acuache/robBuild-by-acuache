...
export default function CardRigthImageDescription({ imageName, titleImage, descriptionImage }: CardRigthImageDescriptionProps) {
  ...
  return (
    <article ref={ref} className='w-full flex flex-col items-center lg:flex-row lg:relative  lg:px-8 xl:px-0 lg:justify-end lg:-mt-25'>
      <div className={`z-2 hidden lg:flex flex-col justify-center items-center p-7 gap-4 bg-white lg:absolute lg:h-80 lg:left-0 lg:top-1/2 lg:-translate-y-1/4 sm:p-12 sm:max-w-115 sm:-mt-20 lg:flex-1/3 text-center lg:text-end transition-all duration-1000 ${inView ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-80"}`}>
      ...
      <div className={`z-2 flex flex-col justify-center items-center p-7 gap-4 bg-white lg:absolute lg:h-80 lg:left-0 lg:top-1/2 lg:-translate-y-1/4 sm:p-12 sm:max-w-115 sm:-mt-20 lg:flex-1/3 text-center lg:text-end transition-all duration-1000 ${inView ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-80"}`}>
        <h3 className='text-2xl  font-semibold sm:text-3xl'>{titleImage}</h3>
        <p className='text-md  sm:text-lg'>{descriptionImage}</p>
      </div>
